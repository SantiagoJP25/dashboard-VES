...
# Limpiar datos
df_dur = df_dur.dropna(subset=["INICIO_DT", "FIN_DT"]).copy()
df_dur = df_dur[df_dur["FIN_DT"] >= df_dur["INICIO_DT"]]

# Redondear a hora
df_dur["INICIO_H"] = df_dur["INICIO_DT"].dt.floor("h")
df_dur["FIN_H"] = df_dur["FIN_DT"].dt.floor("h")

# Expandir sin apply (CLAVE)
horas = df_dur.loc[
    df_dur.index.repeat(
        ((df_dur["FIN_H"] - df_dur["INICIO_H"]).dt.total_seconds() // 3600 + 1)
    )
].copy()
...
horas["HORA"] = (
    horas["INICIO_H"]
    + pd.to_timedelta(horas.groupby(level=0).cumcount(), unit="h")
)
